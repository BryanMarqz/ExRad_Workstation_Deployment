...
$AutoHotkeyFile = Get-NewestVersionedFileName `
    -Filter 'AutoHotkey_*_setup.exe' `
    -VersionPattern '^AutoHotkey_(?<Version>2(?:\.\d+)+)_setup\.exe$' `
    -Fallback 'AutoHotkey_2_setup.exe'
...
$Apps = @(
    ...
    @{
        Name = 'NinjaOne Agent'
        DisplayNote = 'opt-in; install after imaging'
        HelpText = 'Golden image warning: install NinjaOne after deploying the image to avoid cloning an existing device identity.'
        CheckPath = "${env:ProgramFiles(x86)}\NinjaOne\NinjaRMMAgent.exe"
        Type = 'MSI'
        File = $NinjaFile
        Args = '/qn /norestart'
